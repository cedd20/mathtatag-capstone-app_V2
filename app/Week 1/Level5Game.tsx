...
const Level5Game: React.FC<Level5GameProps> = ({ onComplete, onExit }) => {
  ...
  const elementAnimations = useRef<{[key: number]: Animated.Value}>({
    1: new Animated.Value(1), // Book A (thinnest)
    2: new Animated.Value(1), // Book B (medium)
    3: new Animated.Value(1), // Book C (thickest)
  }).current;
  ...
  const correctAnswer = 3; // Book C is the thickest
  ...
  const startElementAnimations = useCallback(() => {
    ...
    const animations = [
      // Book A animation - gentle bounce
      Animated.loop(
        Animated.sequence([
        ...
        if (isResizing && resizeHandle) {
          ...
          setElements(prev => prev.map(element => {
            if (element.id === elementId) {
              ...
              return {
                ...
                width: newWidth,
                height: newHeight,
                x: Math.max(-50, Math.min(width - newWidth + 50, newX)),
          ...
          setElements(prev => prev.map(element => 
            element.id === elementId 
              ? {
                  ...element,
                  x: Math.max(-50, Math.min(width - element.width + 50, element.x + gestureState.dx)),
  ...
  const saveLayout = async () => {
    try {
      ...
      await AsyncStorage.setItem('level5_layout', JSON.stringify(layoutData));
  ...
  const loadSavedLayout = async () => {
    try {
      ...
      const savedLayout = await AsyncStorage.getItem('level5_layout');
  ...
  const clearSavedLayout = async () => {
    try {
      await AsyncStorage.removeItem('level5_layout');
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Text style={styles.title}>Level 5: Hanapin ang Pinakamakapal!</Text>
      ...
      {showResult && (
        <Animated.View style={styles.resultContainer}>
          ...
          <Text style={styles.resultExplanation}>
            {isCorrect 
              ? 'Tama! Ang Aklat C ang pinakamakapal sa tatlong aklat.'
              : 'Ang Aklat C ang pinakamakapal sa tatlong aklat.'
            }
          </Text>

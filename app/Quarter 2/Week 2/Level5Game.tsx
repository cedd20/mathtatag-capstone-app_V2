...
const Level5Game: React.FC<Level5GameProps> = ({ onComplete, onExit }) => {
  ...
  const elementAnimations = useRef<{[key: number]: Animated.Value}>({
    1: new Animated.Value(1), // Rock A
    2: new Animated.Value(1), // Book B
    3: new Animated.Value(1), // Feather C
  }).current;
  ...
  const defaultElements = useMemo(() => [
    {
      ...
      height: 80,
      scale: 1,
      image: require('../../../assets/Quarter 2/Week 1/b1.png'), // Using Week 1 assets as placeholder
      name: 'Bato A',
      emoji: '🪨'
    },
    {
      ...
      height: 80,
      scale: 1,
      image: require('../../../assets/Quarter 2/Week 1/b2.png'), // Using Week 1 assets as placeholder
      name: 'Libro B',
      emoji: '📚'
    },
    {
      ...
      height: 80,
      scale: 1,
      image: require('../../../assets/Quarter 2/Week 1/b3.png'), // Using Week 1 assets as placeholder
      name: 'Balahibo C',
      emoji: '🪶'
    },
  ], []);
  ...
  const correctAnswer = 1; // Rock A is the heaviest
  ...
  const saveLayout = async () => {
    try {
      ...
      await AsyncStorage.setItem('week2_level5_layout', JSON.stringify(layoutData));
  ...
  const loadSavedLayout = async () => {
    try {
      ...
      const savedLayout = await AsyncStorage.getItem('week2_level5_layout');
  ...
  const clearSavedLayout = async () => {
    try {
      await AsyncStorage.removeItem('week2_level5_layout');
  ...
  return (
    <ImageBackground 
      source={require('../../../assets/Quarter 2/Week 1/bgLevel5.png')} // Placeholder - will be updated with Week 2 assets
      style={styles.container} 
      resizeMode="cover"
    >
      <View style={styles.header}>
        <Text style={styles.title}>Level 5: Hanapin ang Pinakamabigat!</Text>
      ...
      {!showQuestion ? (
        <View style={styles.instructionContainer}>
          <Text style={styles.instruction}>
            Si Ben ay nag-aayos ng kanyang mga gamit sa kahon. May tatlong bagay siyang nakita: A - Bato, B - Libro, at C - Balahibo. Bawat isa ay may iba't ibang bigat.
          </Text>
          <Text style={styles.question}>
            Tulongan si Ben! Alin kaya sa mga bagay ang pinakamabigat?
          </Text>
